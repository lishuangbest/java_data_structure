...
/**
 * @Auther: 李爽
 * @Date: 2018/12/28 09:59
 * @Description: 二叉树存储结构结点定义
 */
public class BinTreeNode implements Node {
    ...
    public BinTreeNode setLChild(BinTreeNode lc) {
        ...
        if (hasLChild())
            lChild.server();    //断开当前左孩子与结点的关系
        if (lc!=null){
            lc.server();        //断开lc与其父结点的关系
    ...
    public BinTreeNode setRChild(BinTreeNode rc){
        ...
        if (hasRChild())
            rChild.server();    //断开当前右孩子与结点的关系
        if (rc!=null){
            rc.server();        //断开rc与其父结点的关系
